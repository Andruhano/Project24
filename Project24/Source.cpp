...
class Car 
{
    ...
    string color;
public:
    Car(const string& mdl, Engine* eng, Transmission* trans, Manufacturer* manu, const string& body, const string& drive, const string& col) : model(mdl), engine(dynamic_cast<Engine*>(eng->clone())), transmission(dynamic_cast<Transmission*>(trans->clone())), manufacturer(dynamic_cast<Manufacturer*>(manu->clone())), bodyType(body), driveType(drive), color(col) {}
    Car(const Car& other) : model(other.model), bodyType(other.bodyType), driveType(other.driveType), color(other.color) 
    {
...
int main() 
{
    ...
    Car bmw("BMW M5 F90", &bmwEngine, &bmwTransmission, &bmwManufacturer, "Sedan", "AWD", "Black");
    Car vw("Volkswagen Passat B7", &vwEngine, &vwTransmission, &vwManufacturer, "Universal", "FWD", "White");
    Car tesla("Tesla Model X", &teslaEngine, &teslaTransmission, &teslaManufacturer, "Liftback", "AWD", "Red");
    Car toyota("Toyota Camry", &toyotaEngine, &toyotaTransmission, &toyotaManufacturer, "Sedan", "FWD", "Black");
    Car audi("Audi RS6", &audiEngine, &audiTransmission, &audiManufacturer, "Universal", "AWD", "Gray");
    Car mercedes("Mercedes Benz W213", &mercedesEngine, &mercedesTransmission, &mercedesManufacturer, "Sedan", "AWD", "Dark Blue");
    Car subaru("Subaru Impreza WRX STI", &subaruEngine, &subaruTransmission, &subaruManufacturer, "Sedan", "AWD", "Black-red");
